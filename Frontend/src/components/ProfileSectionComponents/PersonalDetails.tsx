...
const PersonalDetails: React.FC<PersonalDetailsProps> = ({ onChange }) => {
  ...
  const handleChange = (field: keyof typeof formData, value: string | Date | null) => {
    ...
    if (onChange) onChange(updatedData); // Pass data up if needed
  };
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-8 gap-4 items-center">
        ...
        <div className="md:col-span-3 col-span-1">
          <DatePickerDemo 
            onDateChange={(date) => handleChange("dateOfBirth", date)} // Handle date change
          />
        </div>
        ...
        <div className="flex flex-col items-start col-span-1 md:col-span-3">
          <RadioGroup
            ...
            onChange={(e) => handleChange("gender", (e.target as HTMLInputElement).value)}
